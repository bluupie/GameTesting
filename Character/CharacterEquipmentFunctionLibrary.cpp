...
FEquipOutcome UCharacterEquipmentFunctionLibrary::EquipItem(FCharacterEquipment& Equipment, EEquipmentSlot Slot,
                                                              const FGearItem& Item, int32 CharacterLevel)
{
    ...
    if (!MeetsLevelRequirement(Item, CharacterLevel))
    {
        Outcome.Result = EEquipResult::Failed_LevelRequirement;
        return Outcome;
    }
...
FEquipOutcome UCharacterEquipmentFunctionLibrary::UnequipSlot(FCharacterEquipment& Equipment, EEquipmentSlot Slot)
{
    ...
    {
        Equipment.ClearOffHandMetadata();
    }
    Outcome.Result = EEquipResult::Success;
    return Outcome;
}
...
FMainHandEquipOutcome UCharacterEquipmentFunctionLibrary::EquipMainHandWeapon(FCharacterEquipment& Equipment, const FGearItem& WeaponItem,
                                                                               EWeaponCategory WeaponCategory, int32 CharacterLevel)
{
    ...
    if (!MeetsLevelRequirement(WeaponItem, CharacterLevel))
    {
        Outcome.Result = EEquipResult::Failed_LevelRequirement;
        return Outcome;
    }
    ...
    else if (const FGearItem* CurrentOffHand = Equipment.GetEquippedItem(EEquipmentSlot::OffHand))
    {
        ...
        bool bStillValid = Equipment.bHasOffHandMetadata &&
    ...
    Equipment.SetMainHandMetadata(WeaponCategory);
    Outcome.Result = EEquipResult::Success;
    return Outcome;
}
...
FMainHandEquipOutcome UCharacterEquipmentFunctionLibrary::UnequipMainHandWeapon(FCharacterEquipment& Equipment)
{
    ...
    Equipment.ClearMainHandMetadata();
    ...
    {
        if (!Equipment.bHasOffHandMetadata || Equipment.CurrentOffHandKind != EOffHandItemKind::Shield)
        {
            Outcome.bHadBumpedOffHand = true;
            Outcome.BumpedOffHand = *CurrentOffHand;
            Equipment.EquippedItems.Remove(EEquipmentSlot::OffHand);
                                                                 ...
                                                                 int32 CharacterLevel)
{
    ...
    if (!MeetsLevelRequirement(OffHandItem, CharacterLevel))
    {
        Outcome.Result = EEquipResult::Failed_LevelRequirement;
        return Outcome;
    }
    ...
    {
        if (!Equipment.bHasMainHandWeaponMetadata)
        {
    ...
    Equipment.SetOffHandMetadata(ItemKind, OffHandWeaponCategory);
    Outcome.Result = EEquipResult::Success;
    return Outcome;
}

bool UCharacterEquipmentFunctionLibrary::MeetsLevelRequirement(const FGearItem& Item, int32 CharacterLevel)
{
    return CharacterLevel >= FMath::Max(1, Item.RequiredCharacterLevel);
}
